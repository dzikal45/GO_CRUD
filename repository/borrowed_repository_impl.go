...
func (repository *BorrowedByRepositoryImpl) Save(ctx context.Context, tx *sql.Tx, borrowed domain.BorrowedBy) domain.BorrowedBy {
	sql := "insert into borrowed (student_id,status_request,book_name,return_date,due_date) values(?,?,?,?,?)"
	result, err := tx.ExecContext(ctx, sql, borrowed.StudentId, borrowed.StatusRequest, borrowed.BookName, borrowed.ReturnDate, borrowed.DueDate)
...
func (repository *BorrowedByRepositoryImpl) FindById(ctx context.Context, db *sql.DB, borrowedId int) (domain.BorrowedBy, error) {
	sql := "Select borrowed_id,student_id,staus_request,book_name,return_date,due_date from borrowed where borrowed_id = ?"
	rows, err := db.QueryContext(ctx, sql, borrowedId)
	helper.PanicIfError(err)
	borrowed := domain.BorrowedBy{}
	defer rows.Close()
	if rows.Next() {
		err := rows.Scan(&borrowed.BorrowedId, &borrowed.StudentId, &borrowed.StatusRequest, &borrowed.BookName, &borrowed.ReturnDate, &borrowed.DueDate)
		helper.PanicIfError(err)
		return borrowed, nil
	} else {
...
func (repository *BorrowedByRepositoryImpl) FindAll(ctx context.Context, db *sql.DB) []domain.BorrowedBy {
	sql := "select borrowed_id,student_id,status_request,book_name,return_date,due_date from borrowed"
	rows, err := db.QueryContext(ctx, sql)
	helper.PanicIfError(err)
	defer rows.Close()
	borrows := []domain.BorrowedBy{}
	for rows.Next() {
		borrowed := domain.BorrowedBy{}
		err := rows.Scan(&borrowed.BorrowedId, &borrowed.StudentId, &borrowed.StatusRequest, &borrowed.BookName, &borrowed.ReturnDate, &borrowed.DueDate)
		helper.PanicIfError(err)
